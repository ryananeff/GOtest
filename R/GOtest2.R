GOtest2=function(x, go, query.population=NULL, background='common', name.x='Input', name.go='Category', method=c('hypergeometric','GSEA','logitreg'), adj="BH", ReportOverlap=TRUE, ncores=1, gsea.alpha=1, permutations=ifelse(method=='GSEA',1000,0), iseed=12345){
...
#query.population, a vector of background genes in where x is derived
	...
	if(method=='hypergeometric'){
		...
		EnrichTable <- callGSEAfunc3(Set1=go, Set2=x, alpha=gsea.alpha, permutations=permutations, ncores=ncores, iseed=iseed)
	}else{
